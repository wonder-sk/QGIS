...
QgsBox3D globeNodeIdToBox3D( QgsChunkNodeId n )
{
...
class QgsGlobeChunkLoader : public QgsChunkLoader
{
  public:
    ...
    Qt3DCore::QEntity *createEntity( Qt3DCore::QEntity *parent ) override
    {
      ...
      // TODO: figure out how many slices to use based on the node's error
...
class QgsGlobeChunkLoaderFactory : public QgsChunkLoaderFactory
{
  public:
    ...
    {
      const double radius = 6'400'000;  // just an approximate value for Earth's radius
      QgsBox3D rootNodeBox3D( -radius, -radius, -radius, +radius, +radius, +radius );
    ...
    QVector<QgsChunkNode *> createChildren( QgsChunkNode *node ) const override
    {
      ...
      if ( node->tileId().d == 0 )
      {
        double radius = 6'400'000;
        QgsBox3D boxWest( -radius, -radius, -radius, +radius, 0, +radius );
        QgsBox3D boxEast( -radius, 0, -radius, +radius, +radius, +radius );
        float error = 30'000;  // TODO
      ...
      else if ( node->tileId().d < 20 )  // TODO: calculate max. depth from max. terrain ground error
      {
        QgsChunkNodeId nid = node->tileId();
        float cerror = node->error() / 2;
        double latMin, latMax, lonMin, lonMax;
        ...
        children << new QgsChunkNode( cid1, globeNodeIdToBox3D( cid1 ), cerror, node )
                 << new QgsChunkNode( cid2, globeNodeIdToBox3D( cid2 ), cerror, node )
                 << new QgsChunkNode( cid3, globeNodeIdToBox3D( cid3 ), cerror, node )
                 << new QgsChunkNode( cid4, globeNodeIdToBox3D( cid4 ), cerror, node );
      }
